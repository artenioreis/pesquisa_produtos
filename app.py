...
def conectar_banco():
    """Estabelece conexão com o banco de dados usando configurações salvas"""
...
@app.route('/')
def index():
    """Página inicial - redireciona para conexão se não houver configuração"""
    config = carregar_config()
    if 'database' not in config:
...
def buscar_produto():
    """Tela de busca de produtos - busca consolidada por código, EAN ou nome"""
    conn = conectar_banco()
    if not conn:
        return redirect(url_for('conexao'))
...
def detalhes_produto(codigo):
    """Exibe detalhes do produto: estoque atual e últimas entradas em uma única página"""
    conn = conectar_banco()
    if not conn:
        return redirect(url_for('conexao'))
    ...
    try:
        ...
        cursor.execute("""
            SELECT Codigo, Descricao, Cod_EAN 
            FROM PRODU 
            WHERE Codigo = ?
        """, codigo)
        ...
        produto = {
            ...
            'descricao': produto_info[1],
            'cod_ean': produto_info[2]
        }

        # 2. Definir datas para filtro de entradas
        data_fim = datetime.now()
        data_inicio = data_fim - timedelta(days=90)

        # Se for POST, usar datas do formulário
        if request.method == 'POST':
            ...
            if data_inicio_form:
                try:
                    data_inicio = datetime.strptime(data_inicio_form, '%Y-%m-%d')
                except:
                    pass

            if data_fim_form:
                try:
                    data_fim = datetime.strptime(data_fim_form, '%Y-%m-%d')
                except:
                    pass
        ...
        # 3. Consulta de estoque atual (PRIMEIRA INFORMAÇÃO)
        query_estoque = """
        SELECT
            pd.Codigo AS Cod_Produto,
            pd.Descricao AS Desc_Produto,
            dp.Cod_Lote,
            dp.Dat_Vencim,
            Qtd_Fisico = CASE 
                            WHEN IsNull(pm.FlgExbEstPrcAprPrd, 0) = 1 THEN dp.Qtd_Fisico / IsNull(ap.Fat_CnvApr, 1)
                            ELSE dp.Qtd_Fisico 
                         END,
            Qtd_Solicitado = CASE 
                                WHEN IsNull(pm.FlgExbEstPrcAprPrd, 0) = 1 THEN dp.Qtd_Solicitado / IsNull(ap.Fat_CnvApr, 1)
                                ELSE dp.Qtd_Solicitado 
                             END,
            dp.Cod_Dep AS Deposito,
            Loc_Fis = dbo.FN_FormataEndereco(dp.Num_Rua, dp.Num_Col, dp.Num_Niv, dp.Num_Apt),
            cl.Descricao AS Linha
        FROM PRLTL dp
        INNER JOIN PARAM pm ON dp.Cod_Estabe = pm.Cod_Estabe
        INNER JOIN PRODU pd ON dp.Cod_Produt = pd.Codigo
        ...
        SELECT 
            ...
            fr.Dat_Vencim,
            Qtd_Fisico = CASE 
                            WHEN IsNull(pm.FlgExbEstPrcAprPrd, 0) = 1 THEN fr.Qtd_Fisico / IsNull(ap.Fat_CnvApr, 1)
                            ELSE fr.Qtd_Fisico 
                         END,
            Qtd_Solicitado = CASE 
                                WHEN IsNull(pm.FlgExbEstPrcAprPrd, 0) = 1 THEN fr.Qtd_Solicitado / IsNull(ap.Fat_CnvApr, 1)
                                ELSE fr.Qtd_Solicitado 
                             END,
            fr.Cod_Dep AS Deposito,
            Loc_Fis = d.Cod_LocFis,
            cl.Descricao AS Linha
        FROM PRLOT fr
        INNER JOIN PARAM pm ON fr.Cod_Estabe = pm.Cod_Estabe
        INNER JOIN PRODU p ON fr.Cod_Produt = p.Codigo
        INNER JOIN DPXPR d ON (fr.Cod_Estabe = d.Cod_Estabe AND fr.Cod_Dep = d.Cod_Dep AND fr.Cod_Produt = d.Cod_Produt)
        LEFT OUTER JOIN TBZON z ON (fr.Cod_Estabe = z.Cod_Estabe AND fr.Cod_Dep = z.Cod_Dep AND d.Num_Rua BETWEEN z.Num_RuaIni AND z.Num_RuaFim)
        ...
        # 4. Consulta das últimas entradas (SEGUNDA INFORMAÇÃO) com filtro de data
        query_entradas = """
        SELECT 
            cb.Cod_Estabe,
            cb.Protocolo,
            ...
            cb.Dat_Emissao,
            it.Dat_Movimento,
            (IsNull(it.Qtd_Pedido, 0) + IsNull(it.Qtd_Bonificacao, 0)) AS C_Quantidade,
            it.Qtd_Bonificacao,
            it.Cod_Lote,
            C_DesEmitente = CASE
                                WHEN cb.Cod_EmiFornec > 0 THEN (SELECT Razao_Social FROM FORNE WHERE Codigo = cb.Cod_EmiFornec)
                                WHEN cb.Cod_EmiCliente > 0 THEN (SELECT Razao_Social FROM CLIEN WHERE Codigo = cb.Cod_EmiCliente)
                                WHEN cb.Cod_EmiTransp > 0 THEN (SELECT Razao_Social FROM TRANS WHERE Codigo = cb.Cod_EmiTransp)
                                ELSE ''
                            END,
            (x.Des_UnvApr + ' ' + x.Des_Apr) AS C_DesApr,
            Qtd_ComApr = CASE
                            WHEN x.Fat_CnvApr > 0 THEN Round((IsNull(it.Qtd_Pedido, 0) + IsNull(it.Qtd_Bonificacao, 0)) * 1.0 / x.Fat_CnvApr, 2)
                            ELSE 0
                         END
        FROM NFECB cb
        ...
        WHERE cb.Cod_Estabe IN (0)
            AND cb.Status = 'F'
            AND it.Cod_Produto = ?
            AND it.Dat_Movimento >= ?
            AND it.Dat_Movimento <= ?
            AND cb.Tip_NF = 'C'
        ...
        # Passar data atual para o template
        data_atual = datetime.now()

        return render_template('resultado_produto.html',
                             produto=produto,
                             estoque=estoque,
                             entradas=entradas,
        ...
        print(f"Erro ao buscar detalhes do produto: {str(e)}")
        traceback.print_exc()
...
def testar_conexao():
    ...
    try:
        conn_str = (
            ...
            f"UID={data.get('username', '')};"
            f"PWD={data.get('password', '')}"
        )
        conn = pyodbc.connect(conn_str)
        conn.close()
